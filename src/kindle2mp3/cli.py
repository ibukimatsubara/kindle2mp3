...
def build_parser() -> argparse.ArgumentParser:
    ...
    llm_fix_run.add_argument("--model", default=None, help="Gemini model name")
    ...
    podcast_parser = subparsers.add_parser("podcast", help="Generate and serve podcast RSS feed")
    podcast_subparsers = podcast_parser.add_subparsers(dest="podcast_command", required=True)

    podcast_generate = podcast_subparsers.add_parser("generate", help="Generate RSS feed from completed sessions")
    podcast_generate.add_argument("--output-dir", default="podcast", help="Directory for feed.xml and media files")
    podcast_generate.add_argument("--base-url", default="http://localhost:8080", help="Base URL for enclosure links")
    podcast_generate.add_argument("--title", default="kindle2mp3", help="Podcast title")
    podcast_generate.add_argument("--description", default="Audiobooks generated by kindle2mp3", help="Podcast description")

    podcast_serve = podcast_subparsers.add_parser("serve", help="Serve podcast feed over HTTP")
    podcast_serve.add_argument("--output-dir", default="podcast", help="Directory containing feed.xml")
    podcast_serve.add_argument("--base-url", default="http://localhost:8080", help="Base URL for enclosure links")
    podcast_serve.add_argument("--title", default="kindle2mp3", help="Podcast title")
    podcast_serve.add_argument("--description", default="Audiobooks generated by kindle2mp3", help="Podcast description")
    podcast_serve.add_argument("--host", default="0.0.0.0", help="Bind address")
    podcast_serve.add_argument("--port", type=int, default=8080, help="Port number")
...
def handle_podcast(args: argparse.Namespace) -> int:
    from kindle2mp3.podcast import generate_feed, serve

    if args.podcast_command not in ("generate", "serve"):
        print(f"error: unknown podcast subcommand: {args.podcast_command}", file=sys.stderr)
        return 2

    manager = SessionManager()
    output_dir = Path(args.output_dir)

    feed_path, episode_count = generate_feed(
        manager=manager,
    ...
    print(f"Generated: {feed_path} ({episode_count} episode(s))")

    if args.podcast_command == "serve":
        if episode_count == 0:
            print("error: no completed sessions with audio found — nothing to serve", file=sys.stderr)
            return 1
        serve(directory=output_dir, host=args.host, port=args.port)
